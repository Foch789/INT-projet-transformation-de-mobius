...
array_distorsion_angles = mt.dist.angle_difference(meshs[0], meshs[9])
array_distorsion_areas = mt.dist.area_difference(meshs[0], meshs[9])
array_distorsion_edges_length = []

for i in range(0, len(meshs[0].faces_unique_edges)):
    res = 0
    for p in meshs[0].faces_unique_edges[i]:
        res += np.abs(meshs[0].edges_unique_length[p]
                      - meshs[9].edges_unique_length[p])
    res = res/3
...
print("Le min de distorsion d'angle : " + str(tex_sum.min())
      + " et le max : " + str(tex_sum.max()))

splt.pyglet_plot(meshs[0],
                 tex_sum,
                 map_min=tex_sum.min(),
                 map_max=tex_sum.max(),
                 plot_colormap=False)

print("Le min de distorsion d'aires : " + str(tex_areas.min())
      + " et le max : " + str(tex_areas.max()))

splt.pyglet_plot(meshs[0],
                 tex_areas.darray,
                 map_min=tex_areas.min(),
                 map_max=tex_areas.max(),
                 plot_colormap=False)

print("Le min de distorsion de longueur d'arête : "
      + str(tex_edges_length.min())
      + " et le max : " + str(tex_edges_length.max()))

splt.pyglet_plot(meshs[0],
                 tex_edges_length.darray,
                 map_min=tex_edges_length.min(),
                 map_max=tex_edges_length.max(),
                 plot_colormap=False)


splt.pyglet_plot(meshs[9], tex_edges_length.darray,
                 map_min=tex_edges_length.min(),
                 map_max=tex_edges_length.max(),
                 plot_colormap=False)
